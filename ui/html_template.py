...
<body> 
    ...
    <script> 
        ...
        // ==================== 双击 Ctrl：将选中文字送入搜索框并搜索 ====================
        (function () {
            var lastCtrlTime = 0;
            var resetTimer = null;
            var DOUBLE_CTRL_DELAY = 350; // 两次 Ctrl 按下的最大间隔(ms)
            ...
            // 获取当前选中的文本：依次检查主文档选区、输入框选区、iframe 选区
            ...
            function handleDoubleCtrl() {
                ...
                if (input) {
                    ...
                    input.focus();
                    // 将光标移到末尾
                    try { input.setSelectionRange(selected.length, selected.length); } catch (e) {}
                }
                triggerSearch();
            }
            ...
            document.addEventListener('keydown', function (e) {
                ...
                if (e.key === 'Control' || e.keyCode === 17) {
                    var now = Date.now();
                    if (lastCtrlTime > 0 && (now - lastCtrlTime) < DOUBLE_CTRL_DELAY) {
                        // 第二次 Ctrl 在阈值内 → 触发双击
                        lastCtrlTime = 0;
                        clearResetTimer();
                        handleDoubleCtrl();
                        ...
                        lastCtrlTime = now;
                        clearResetTimer();
                        resetTimer = setTimeout(function () { lastCtrlTime = 0; }, DOUBLE_CTRL_DELAY + 50);
                    }
                } else {
                    // 按下其他键则重置双击计时，避免与 Ctrl+C 等组合键冲突
                    lastCtrlTime = 0;
                    clearResetTimer();
                }
            }, true); // 使用捕获阶段，尽早拦截
