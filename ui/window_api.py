        ...
        def task():
            ...
            if header_info:
                ...
                parts.append(f"<p style='margin-top:6px;'><b>标题:</b> {html_module.escape(disp_title)} "
                             f"<span style='color:#888;font-size:12px;'>{label}</span></p>")
                ...
                if tech:
                    parts.append(f"<p style='margin-top:8px;'><b>技术参数:</b></p>"
                                 f"<ul style='margin-left:20px;font-size:13px;'>{''.join(tech)}</ul>")

                raw = header_info.get("raw_header", {})
                if raw:
                    parts.append("<details style='margin-top:8px;'>"
                                 "<summary style='cursor:pointer;color:#2196F3;font-size:13px;'>全部 Header 字段</summary>"
